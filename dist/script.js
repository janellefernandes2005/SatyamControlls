...
let currentIndex = 0;
let slideInterval;
// Background colors for categories (YOUR ORIGINAL COLORS)
const backgroundColors = ["#C1D1CF", "#E2E8E2", "#B5BFC6", "#D6E4F0", "#E8D7F1", "#F0E6D2", "#D4E6F1"];
// Image error handler
const handleImageError = (img, text) => {
    console.log(`❌ Image failed: ${text}`);
    img.src = `/api/products/image/6975126cd0195fb59dbeeeed`; // FIXED: Use working MongoDB image
};
...
const loadMainCategories = async () => {
    try {
        ...
        // Add background colors (YOUR ORIGINAL CODE)
        mainCategories = categories.map((category, index) => ({
            id: category.id,
            name: category.name,
            desc: category.desc || `Explore our ${category.name} collection`,
            imageUrl: `/api/products/main-category-image/${category.id}`, // FIXED: Use API endpoint
        ...
        // Fallback with working images (YOUR ORIGINAL FALLBACK)
        mainCategories = [
            {
                id: "tube-fittings",
                name: "Tube Fittings",
            ...
            },
            {
                ...
                imageUrl: "/api/products/image/6975126cd0195fb59dbeef07",
                count: 0,
                bg: "#B5BFC6"
            },
            {
                ...
                imageUrl: "/api/products/image/6975126dd0195fb59dbeef15",
                count: 0,
                bg: "#D6E4F0"
            },
            {
                ...
                imageUrl: "/api/products/image/6975126cd0195fb59dbeeed3",
                count: 0,
                bg: "#b8b2ba"
            },
            {
                ...
                imageUrl: "/api/products/image/6975126cd0195fb59dbeef01",
                count: 0,
                bg: "#ada89f"
            },
            {
                ...
                imageUrl: "/api/products/image/6975126cd0195fb59dbeeee9",
                count: 0,
                bg: "#99a1a0"
            }
        ];
...
const initializeCarousel = () => {
    ...
    // Set main image (YOUR ORIGINAL CODE)
    if (activeProductImg) {
    ...
    // Clear track (YOUR ORIGINAL CODE)
    if (track) {
        track.innerHTML = '';
    }
    // Create category cards with YOUR ORIGINAL STYLES
    ...
    // Update display function (YOUR ORIGINAL CODE)
    const updateDisplay = () => {
        if (mainCategories.length === 0)
            return;
        const category = mainCategories[currentIndex];
        if (textContent)
            textContent.classList.add('hero-hidden');
        if (activeProductImg)
            activeProductImg.classList.add('hero-hidden');
        if (track) {
        ...
        setTimeout(() => {
            ...
            requestAnimationFrame(() => {
                if (textContent)
                    textContent.classList.remove('hero-hidden');
                if (activeProductImg)
                    activeProductImg.classList.remove('hero-hidden');
            });
        }, 150);
    };
    // Auto slide function (YOUR ORIGINAL CODE)
    const autoSlide = () => {
        if (mainCategories.length === 0)
            return;
    ...
    // Start timer function (YOUR ORIGINAL CODE - 8 seconds)
    const startTimer = () => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (prodTitle)
        prodTitle.textContent = "Loading...";
    if (prodDesc)
        prodDesc.textContent = "Loading categories...";
    if (activeProductImg) {
