...
async function fetchProductsFromMongoDB(): Promise<Record<string, Category>> {
    try {
        ...
        products.forEach((product: any) => {
            if (!grouped[product.category]) {
                grouped[product.category] = {
                    name: product.category.split('-').map((word: string) => 
...
async function getProductsByCategory(category: string): Promise<ProductDetail[]> {
    try {
...
async function initializePage(): Promise<void> {
    ...
    try {
        ...
        if (products.length > 0) {
            // Use MongoDB data
            const categoryName = catId.split('-').map(word => 
                word.charAt(0).toUpperCase() + word.slice(1)
            ).join(' ');
